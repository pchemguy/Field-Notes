...
### PoC vs MVP is not a “size” distinction
...
### Prototype code has three legitimate categories (do not collapse them)

1. **Throwaway prototype:** fast, disposable; used for learning or UI exploration.
2. **Tracer / baseline implementation:** intentionally kept as the first end-to-end thread; minimal but structured.
3. **Spike with reusable core:** exploratory but the *kernel* is wrapped behind stable interfaces and carried forward.

Your notes correctly push toward (2) and (3): “probe” code can become the **baseline** if you contain it behind clean contracts.
...
This is the transition point you were aiming to formalize. It is not “coding” in the product sense, but it **produces code artifacts that often remain**.
...
### Phase 7: Tracer/baseline implementation (bridge into coding)

**Objective:** produce a minimal end-to-end “steel thread” that is architecturally real, even if internally crude.

This is your “not-throwaway feasibility probing” done correctly.
...
# Iteration model: how you evolve PoC → MVP without thrash
...
Your “Ship of Theseus” approach is correct **only if** contracts and artifacts are enforced. Without that, “incremental replacement” becomes accidental coupling.

---

## What to change in your notes (critical assessment)

### 1. Add explicit gates and stop conditions

Your material has strong sequences but not enough “stop/go” enforcement. Senior practice is gatekeeping:

* No domain/data model → architecture selection is not authorized.
* No risk register → feasibility is not authorized.
* No baseline steel thread → optimization/refactor is not authorized.

### 2. Make data modeling non-negotiable and earlier

You correctly identified this gap. The framework above treats domain/data as a foundational pillar, not documentation.

### 3. Replace “Feasibility Validation” with “Risk-driven experiments + baseline thread”

Your refinement (throwaway vs tracer) is right; the correct synthesis is:

* **Spikes** for learning,
* **Tracer/baseline** for iteration,
* **Reusable kernel** when it’s cheap to wrap.

### 4. Treat diagnostics as part of definition of done

You mention diagnostics, but the framework must enforce it:

* A block is not “done” until it produces artifacts that let you explain failures quickly.
