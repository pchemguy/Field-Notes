...
5. `micromamba.exe` may fail silently without spitting any errors or console output, though when this behavior was observed, %ERRORLEVEL% was set to non-zero. It appears that some corrupt or invalid data created in `%APPDATA%\mamba` or under `%USERPROFILE%` may have caused. Micromamba DOES NOT need ANYTHING whatsoever within system directories, when solely used for creation of a new environment within a specified prefix. Make sure to delete any such files and directories (directory names will likely use "mamba" instead of "micromamba").
